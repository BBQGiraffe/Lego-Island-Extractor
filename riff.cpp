...
RIFFFile *riff_init() {
    ...
    r = (RIFFFile*)malloc(sizeof(RIFFFile));
    if(r == NULL) {
...
int riff_grow(RIFFFile *r) {
    ...
    e = (RIFFEntry*)realloc(r->root, sizeof(RIFFEntry) * (r->entryMemCount + 1));
    if(e == NULL) {
...
int riff_entry_seekto(RIFFFile *r, int index) {
    if(fseek(r->f, riff_entry_offset(r, index), SEEK_SET) < 0) {
        return(-1);
    }
        ...
        if(ret == 0) {
            ...
            if(!memcmp(fourCC, MxObFourCC, sizeof(MxObFourCC)) &&
               isMuxed(MxObType)) {
                ...
                pos += 2;
                /* seek past string */
                if(fseek(r->f, unkNameSize, SEEK_CUR) < 0) {
                    fprintf(stderr, "Failed to seek forward.\n");
                    return(-1);
                }
                pos += unkNameSize;
                ...
                if(fseek(r->f, r->root[cur].size, SEEK_CUR) < 0) {
