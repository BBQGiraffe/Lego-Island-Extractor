...
MxOb *mxob_grow(Track *t) {
    ...
    m2 = (MxOb*)realloc(t->mxob, sizeof(MxOb) * (t->mxobs + 1));
    if(m2 == NULL) {
...
Chunk *track_grow(Track *t) {
    ...
    c2 = (Chunk*)realloc(t->c, sizeof(Chunk) * (t->chunks + 1));
    if(c2 == NULL) {
...
int dump_song_cb(RIFFFile *r, int dir, int ent, void *priv) {
    ...
    for(i = 0; i < t->mxobs; i++) {
        if(t->mxob[i].trackType == OMNI_TRACK_TYPE_WAVE) {
            ...
            if(fseek(t->mxob[i].out, WAV_DATA_SIZE_OFFSET, SEEK_SET) < 0) {
            ...
            if(fseek(t->mxob[i].out, WAV_FILE_SIZE_OFFSET, SEEK_SET) < 0) {
